...
for sFrame in range(0,int(frames)):
    ...
    out="1,1268,2,"+str(300+id*40)+",3,1335,57,"+str(frames+1+id)+",62,1,87,1,36,1,51,"+str(frames+2+id)+",63,"+str(rfr)+";"

    if(id!=1):
        out+="1,901,2,"+str(300+(id*30))+",3,225,57,"+str(frames+id)+",62,1,87,1,36,1,51,"+str(id+1)+",28,0,29,"+str(-(id-1)*600)+",10,0,30,0,85,2,544,1;"
        out+="1,901,2,"+str(300+(id*30))+",3,225,57,"+str(frames+id)+",62,1,87,1,36,1,51,"+str(id)+",28,0,29,"+str(-900)+",10,0,30,0,85,2,544,1;"  
    
    with open("var.txt","a") as v:
        v.write(out)
        v.close()
with open("var.txt","a") as v:
    xPos=str(float(WIDTH/2)*SCALE+4800)
    yPos=str(((2000-(float(RES/2)*SCALE))))
    v.write("1,1268,2,100,3,100,36,1,51,"+str(frames+2)+",63,0;1,3802,2,"+str(xPos)+",3,"+str(yPos)+",57,1;")
